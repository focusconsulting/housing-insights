...
class CleanerBase(object, metaclass=ABCMeta):
    def __init__(self, meta, manifest_row, cleaned_csv='', removed_csv='',
        ...
        self.census_mapping = {}
        census_reader = HIReader(os.path.join(
            PYTHON_PATH, 'housinginsights/config/crosswalks/DC_census_tract_crosswalk.csv'))
    ...
    def format_date(self, value):
        ...
        try:
            ...
            if value is None or value == self.null_value:
                ...
                logger.warning(
                    "    Unable to format date properly: {}".format(value))
                date = self.null_value
    ...
    def append_tract_label(self, row, row_num,
        ...
        try:
            ...
            # this prints error for many rows with nulls.
            logger.warning(
                '  no matching Tract found for row {}'.format(row_num, row))
        return row
    ...
    def add_mar_id(self, row, address_id_col_name='ADDRESS_ID'):
        ...
        if lat != self.null_value and lon != self.null_value:
            ...
            try:
                ...
                logger.info(
                    "ValueError in Mar for {} - returning none".format(row['Proj_addre']))
                result = None
...
class BuildingPermitsCleaner(CleanerBase):
    ...
    def rename_cluster(self, row):
        if row['NEIGHBORHOODCLUSTER'] == self.null_value:
            ...
            row['NEIGHBORHOODCLUSTER'] = 'Cluster ' + \
                str(row['NEIGHBORHOODCLUSTER'])
            return row
...
class DCTaxCleaner(CleanerBase):
    ...
    def clean(self, row, row_num=None):
        ...
        row['VACANT_USE'] = self.convert_boolean(
            row['VACANT_USE'].capitalize())
        row = self.parse_dates(row)
...
class hmda_cleaner(CleanerBase):
    def clean(self, row, row_num=None):
        row = self.replace_nulls(row, null_values=['', None])
        row = self.parse_dates(row)
        row = self.append_tract_label(
            row, row_num, column_name='census_tract_number')
        return row
