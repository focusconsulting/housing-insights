...
class LoadData(Colleague):
    ...
    def reload_all_from_manifest(self, use_clean=True,
        ...
        :param use_clean: if True, use clean psv files for data loading
        ...
        the db
        """
        data_ids = self._ingestion_mediator.get_list_use_unique_data_ids()

        if drop_tables:
            self._ingestion_mediator.remove_tables(['all'])
    ...
    ###############################
    # methods for calculating the data for zone_facts table
    ###############################
    def recalculate_database(self):
        """
        An alternative to 'rebuild' and 'update_database' methods - if no new
        data has been added but changes to the calculations are made, re-run
        the calculation routines.
        """
        try:
            # self._automap() #not yet used - created for potential use by calculated fields
            self._ingestion_mediator.create_zone_facts_table()
        ...
        for zone_type in zone_types:
            ...
            for field in sorted(summarize_obs_field_args, reverse=True):
                ...
                try:
                    ...
                    if field == 'residential_units':
                        field_values[field] = res_units_by_zone_type[zone_type][
                            'items']
                        continue
            ...
            try:
                ...
                for zone in zone_specifics:  # list of zones in that zone type, i.e. ['Ward 1', 'Ward 2' etc.]
                    ...
                    engine = self._ingestion_mediator.get_engine()
                    with engine.connect() as conn:
                        # TODO - consider trans/rollback logic for this
                        result = conn.execute(q)
                        query_results.append(result)
            except Exception as e:
                logger.error("Couldn't load data for {} into zone_facts".format(
                    zone_type))
