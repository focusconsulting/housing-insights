...
class DataReader(HIReader):
    ...
    def __init__(self, meta, manifest_row, load_from="local"):
        ...
        if "encoding" not in manifest_row:
            logger.warning(
                "  Warning: encoding not found in manifest. " "Falling back to latin-1."
            )
    ...
    def __iter__(self):
        ...
        if self.path_type == "file":
            ...
            with urlopen(self.path) as ftpstream:
                # Always use canonical header fieldname keys:
                reader = DictReader(
                    codecs.iterdecode(ftpstream, self.encoding), self._keys
                )
                header = next(reader)  # Omit header row from output
                for row in reader:
                    self._length += 1
    ...
    def _download_data_file(self):
        ...
        try:
            ...
            logger.info(
                "  File not found. Attempting to download file to disk: " + self.s3_path
            )
            try:
    ...
    def _set_keys(self):
        ...
        if self.path_type == "url":
            logger.info(
                "  File will be read from remote location: '{}'".format(self.path)
            )
            try:
                with urlopen(self.path) as ftpstream:
                    ...
                    if self._contains_leading_bom(_keys[0], encoding=self.encoding):
                        logger.info(
                            "  Leading BOM (byte order mark) found in file '{}'; "
                ...
                logger.error(
                    "  Error: _set_keys(): Header fieldname keys cannot be set; "
            ...
            try:
                with open(self.path, "r", newline="", encoding=self.encoding) as data:
                    ...
                    if self._contains_leading_bom(_keys[0], encoding=self.encoding):
                        ...
                        _keys[0] = self._remove_leading_bom(
                            _keys[0], encoding=self.encoding
                        )
            except OSError as oe:
                logger.error(
                    "  Error: _set_keys(): Header fieldname keys cannot be set; "
    ...
    def _check_include_flag(self, sql_manifest_row):
        ...
        if self.manifest_row["include_flag"] == "use":
            ...
            logger.warning(
                "Skipping data source. {} include_flag is {}".format(
    ...
    def _do_fields_match(self):
        ...
        meta_entry = self.meta[self.destination_table]
        try:
            field_list = meta_entry["fields"]
        except KeyError:
            logger.info(
                '  table "{}" not found in meta data'.format(self.destination_table)
            )
            return False
        ...
        if not success:
            logger.warning(
                "  do_fields_match: {}. '{}' had missing "
                "items:\n{}".format(success, self.destination_table, self.not_found)
            )
        else:
            logger.info(
                "  do_fields_match: {}. meta.json and csv field lists "
                "match completely for '{}'".format(success, self.destination_table)
            )

        return success

    def _verify_actual_columns_exist_in_metadata(
        self, field_list, ignore_extra_columns=False
    ):
        """
        ...
        for data_file_field in self.keys:
            ...
            if not any(
                field.get("source_name", None) == data_file_field
                for field in field_list
            ):
                self.not_found.append(
                    '"{}" in CSV not found in meta'.format(data_file_field)
                )
                success = False
        return success
        ...
        try:
            ...
            if url.scheme == "https":
                ...
                raise RuntimeError(
                    ...
                    "'{}' specified\n"
                    "  Supported schemes: http | https".format(url_str, url.scheme)
                )
            ...
            if response.status != OK:  # i.e. "HTTP/1.1 200 OK"
                raise RuntimeError(
                    "  Error: Cannot connect to URL '{}'\n"
                    "  HTTP Response status code & reason: "
                    "{} {}".format(url_str, response.status, response.reason)
                )
            ...
            raise RuntimeError(
                ...
                "  url.netloc : '{}'\n"
                "  url.path   : '{}'\n".format(
                    url_str, str(he), url.scheme, url.netloc, url.path
                )
            )
    ...
    def _discreet_exception_handler(self, exception_class, exception, tb):
        ...
        for tb_entry in tb_entries:
            logger.error(
                '  File "{}", line {}, in {}\n'
                "    {}".format(tb_entry[0], tb_entry[1], tb_entry[2], tb_entry[3])
            )
        print("{}: {}".format(exception_class.__name__, exception), file=sys.stderr)
    ...
    def _remove_leading_bom(self, str, encoding="utf-8"):
        ...
        if encoding == "utf-8":
            ...
            if str_as_hex.startswith(bom_as_hex):
                ...
                str_new = "".join(
                    "{}".format(chr(int(str_as_hex[i : i + 2], 16)))
