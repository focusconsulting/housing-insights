...
class WmataApiConn(BaseApiConn):
    ...
    def __init__(
        ...
        secretsFileName = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.pardir, "secrets.json")
        )
        ...
        self._available_unique_data_ids = ["wmata_stops", "wmata_dist"]
    ...
    def get_data(
        ...
        **kwargs
        ...
        for u in unique_data_ids:
            if u == "wmata_stops":
                self.get_stops(
                    unique_data_ids=["wmata_stops"],
                    sample=sample,
                    output_type=output_type,
                )
            elif u == "wmata_dist":
                self.get_dist(
                    unique_data_ids=["wmata_dist"],
                    sample=sample,
                    output_type=output_type,
                    db=db,
                )
                ...
                logger.info(
                    "  unique_data_id '%s' not supported by the WmataApiConn", u
                )
    ...
    def get_stops(self, unique_data_ids=None, sample=False, output_type="csv"):
        ...
        if "wmata_stops" in unique_data_ids:
            ...
            if output_type == "csv":
                self._array_to_csv(
                    self.stopsHeader, self.stopsOutput, self.output_paths[u]
                )
    ...
    def get_dist(
        self, unique_data_ids=None, sample=False, output_type="csv", db="local_database"
    ):
        ...
        if "wmata_dist" in unique_data_ids:
            ...
            railResponse = requests.get(
                "https://api.wmata.com/Rail.svc/json/jStations", headers=wmata_headers
            )
            ...
            for idx, row in enumerate(rows):
                ...
                if lat == None or lon == None:
                    logger.warning(
                        "  Lat or Lon not available for project {}".format(
                            project_details["nlihcid"]
                        )
                    )
                ...
                if lat != None and lon != None:
                    ...
                    logger.info(
                        "  Starting processing rail stations for %s",
                        project_details["nlihcid"],
                    )
                    self._find_rail_stations(
                        self.railStations, project_details, radius, sample=sample, db=db
                    )
                    logger.info(
                        "  Completed processing rail stations for project id %s",
                        project_details["nlihcid"],
                    )

                    # find all bus stops within 0.5 miles
                    logger.info(
                        "  Starting processing bus stations for project id %s",
                        project_details["nlihcid"],
                    )
                    self._find_bus_stations(
                        project_details, radius, sample=sample, db=db
                    )
                    logger.info(
                        "  Completed processing bus stations for project id %s",
                        project_details["nlihcid"],
                    )
            ...
            if output_type == "csv":
                self._array_to_csv(
                    self.distHeader, self.distOutput, self.output_paths[u]
                )
    ...
    def _get_all_bus_stations(self):
        ...
        response = requests.get(
            "https://api.wmata.com/Bus.svc/json/jStops", headers=wmata_headers
        )
    ...
    def _get_walking_distance(
        self, srcLat, srcLon, destLat, destLon, db="local_database"
    ):
        ...
        if self.use_cached_distance == True:
            try:
                ...
                columnset = conn.execute(
                    "select column_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME='wmata_dist'"
                )
                columns = []
                for c in columnset:
                    columns.append(c[0])
                ...
                if (
                    "building_lat" in columns
                    and "building_lon" in columns
                    and "stop_or_station_lat" in columns
                    and "stop_or_station_lon" in columns
                ):
                    # See if row exists
                    proj_query = (
                        "select * from wmata_dist where building_lat='"
                        + str(srcLat)
                        + "' and building_lon='"
                        + str(srcLon)
                        + "' and stop_or_station_lat='"
                        + str(destLat)
                        + "' and stop_or_station_lon='"
                        + str(destLon)
                        + "'"
                    )
        ...
        distReqCoords = (
            str(srcLon) + "," + str(srcLat) + ";" + str(destLon) + "," + str(destLat)
        )
        ...
        walkDistResponse = requests.get(
            "https://api.mapbox.com/directions/v5/mapbox/walking/" + distReqCoords,
        ...
        print(
            "Return value: ",
            walkDistResponse.json()["routes"][0]["legs"][0]["distance"],
        )
    ...
    def _find_rail_stations(
        ...
        for idx, station in enumerate(railStations):
            try:
                crow_distance = self._haversine(
                    lat, lon, station["Lat"], station["Lon"]
                )

                if crow_distance < (radiusinmeters / self.meters_per_mile):
                    walkDist = self._get_walking_distance(
                        lat, lon, str(station["Lat"]), str(station["Lon"]), db=db
                    )
                    ...
                    logger.info(
                        "crow: %s. %s is not close enough to calculate walking distance. ",
                        crow_distance,
                        station["Code"],
                    )
    ...
    def _find_bus_stations(
        self, project_details, radiusinmeters, sample=False, db="local_database"
    ):
        lat = project_details["lat"]
        lon = project_details["lon"]
        nlihc_id = project_details["nlihcid"]
        ...
        params = {"Lat": lat, "Lon": lon, "Radius": str(radiusinmeters)}
        response = requests.get(
        ...
        for idx, stop in enumerate(data["Stops"]):
            try:
                ...
                if crow_distance < (radiusinmeters / self.meters_per_mile):
                    walkDist = self._get_walking_distance(
                        lat, lon, str(stop["Lat"]), str(stop["Lon"]), db=db
                    )
                    walkDistMiles = walkDist / self.meters_per_mile
                    logger.info("crow: %s. walking: %s", crow_distance, walkDistMiles)
