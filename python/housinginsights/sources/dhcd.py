...
from housinginsights.sources.models.dhcd import (
    APP_ID,
    TABLE_ID_MAPPING,
    APP_METADATA_FIELDS,
...
class DhcdApiConn(ProjectBaseApiConn):
    ...
    def __init__(self, baseurl=None, proxies=None, database_choice=None, debug=False):
        ...
        self._urls = {
            unique_data_id: "/" + TABLE_ID_MAPPING[self._table_names[unique_data_id]]
        ...
        self._identifier_translation_map = str.maketrans(
            identifier_unallowed_chars, replacement_underscores
        )
    ...
    def _get_metadata(self):
        ...
        output_path_app_metadata = os.path.join(
            output_path_dir, "_dhcd_dfd_app_metadata.csv"
        )
        output_path_table_metadata = os.path.join(
            output_path_dir, "_dhcd_dfd_table_metadata.csv"
        )

        app_metadata_result = self.get(
            "/" + self._app_dbid, params=DhcdApiConn.PARAMS_METADATA
        )
        app_tables_metadata_xml = xml_fromstring(app_metadata_result.text).findall(
            "./table/chdbids/chdbid"
        )
        ...
        for app_table_metadata in app_tables_metadata_xml:
            ...
            if errcode == 0:
                ...
                unique_data_id = None
                if (
                    "dhcd_dfd_" + table_name_snake_case
                    in self._available_unique_data_ids
                ):
                    ...
                    for field_xml in table_metadata_xml_fields:
                        ...
                        if field_xml.find("parentFieldID") is not None:
                            ...
                            if parent_fid in table_metadata[table_dbid]:
                                ...
                                parent_field_label = table_metadata_xml_root.find(
                                    "./table/fields/field[@id='{}']/label".format(
                                        str(parent_fid)
                                    )
                                ).text
                                parent_field_name = (
                                    parent_field_label.lower().translate(
                                        self._identifier_translation_map
                                    )
                                )
                        ...
                        child_fids = (
                            "|".join(child_fids) if len(child_fids) > 0 else None
                        )
                        ...
                        table_metadata[table_dbid][fid]["field_type"] = field_xml.get(
                            "field_type"
                        )
                        table_metadata[table_dbid][fid]["base_type"] = field_xml.get(
                            "base_type"
                        )
                        table_metadata[table_dbid][fid]["appears_by_default"] = (
                            field_xml.find("appears_by_default").text
                        )

                        table_metadata[table_dbid][fid][
                            "composite_field_parent_fid"
                        ] = parent_fid
                        table_metadata[table_dbid][fid][
                            "composite_field_child_fids"
                        ] = child_fids
                        ...
                        if field_xml.find("formula") is not None:
                            table_metadata[table_dbid][fid]["formula"] = field_xml.find(
                                "formula"
                            ).text
                        ...
                        if field_xml.find("choices") is not None:
                            ...
                            for choice in field_xml.findall("./choices/choice"):
                                table_metadata[table_dbid][fid]["choices"] += (
                                    "\n" + choice.text
                                    if len(table_metadata[table_dbid][fid]["choices"])
                                    > 0
                                    else choice.text
                                )
                        ...
                        if table_metadata[table_dbid][fid]["mode"] == "lookup":
                            if field_xml.find("lutfid") is not None:
                                table_metadata[table_dbid][fid]["lookup_target_fid"] = (
                                    field_xml.find("lutfid").text
                                )
                            if field_xml.find("lusfid") is not None:
                                table_metadata[table_dbid][fid]["lookup_source_fid"] = (
                                    field_xml.find("lusfid").text
                                )
                        ...
                        if (
                            table_metadata[table_dbid][fid]["mode"] == "virtual"
                            and table_metadata[table_dbid][fid]["field_type"]
                            == "dblink"
                        ):
                            if field_xml.find("target_dbid") is not None:
                                table_metadata[table_dbid][fid][
                                    "dblink_target_dbid"
                                ] = field_xml.find("target_dbid").text
                            if field_xml.find("target_fid") is not None:
                                table_metadata[table_dbid][fid]["dblink_target_fid"] = (
                                    field_xml.find("target_fid").text
                                )
                            if field_xml.find("source_fid") is not None:
                                table_metadata[table_dbid][fid]["dblink_source_fid"] = (
                                    field_xml.find("source_fid").text
                                )
                        ...
                        if field_xml.find("mastag") is not None:
                            ...
                            if len(fkey_ref) == 2:
                                ...
                                table_metadata[table_dbid][fid]["fkey_table_alias"] = (
                                    fkey_ref[1].lower()
                                )
                            else:
                                table_metadata[table_dbid][fid][
                        ...
                        table_metadata[table_dbid][fid]["field_help"] = field_xml.find(
                            "fieldhelp"
                        ).text
                        ...
                        if unique_data_id is not None and (
                            INCLUDE_ALL_FIELDS[unique_data_id]
                            or table_metadata[table_dbid][fid]["appears_by_default"]
                            == "1"
                        ):
                    ...
                    app_metadata[table_dbid] = OrderedDict(
                        [
                            ...
                            (
                                "single_record_name",
                                table_metadata_xml_orig.find("single_record_name").text,
                            ),
                            (
                                "plural_record_name",
                                table_metadata_xml_orig.find("plural_record_name").text,
                            ),
                    ...
                    if unique_data_id is not None and unique_data_id in self._fields:
                        ...
                        if INCLUDE_ALL_FIELDS[unique_data_id]:
                            self._params[unique_data_id] = (
                                DhcdApiConn.PARAMS_DATA_ALL_FIELDS
                            )
                        else:
                            self._params[unique_data_id] = (
                                DhcdApiConn.PARAMS_DATA_DEFAULT_FIELDS
                            )
        ...
        all_tables_field_metadata = [
            list(field_metadata_row.values())
            for all_field_metadata in table_metadata.values()
            for field_metadata_row in all_field_metadata.values()
        ]
        self.result_to_csv(
            TABLE_METADATA_FIELDS, all_tables_field_metadata, output_path_table_metadata
        )
        self.result_to_csv(
            APP_METADATA_FIELDS,
    ...
    def get_data(self, unique_data_ids=None, sample=False, output_type="csv", **kwargs):
        ...
        for u in unique_data_ids:
            if u not in self._available_unique_data_ids:
                logger.info(
                    "  The unique_data_id '{}' is not supported by the DhcdApiConn".format(
                        u
                    )
                )
                ...
                results = [
                    DhcdResult({e.tag: e.text for e in list(r)}, self._fields[u]).data
