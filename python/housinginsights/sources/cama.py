...
class MarApiConn_2(BaseApiConn):
    ...
    def get_data(self, square, lot, suffix):
        ...
        params = {"f": "json", "Square": square, "Lot": lot, "Suffix": suffix}
        ...
        if mar_data["returnDataset"] == {}:
            mar_returns = {
                "Warning": "No MAR data availble - property under construction - see AYB year"
            }
        else:
...
class CamaApiConn(BaseApiConn):
    ...
    def get_data(self):
        ...
        result = self.get(
            urlpath="/c5fb3fbe4c694a59a6eef7bf5f8bc49a_25.geojson", params=None
        )
        ...
        for index, row in enumerate(cama_data["features"]):
            ...
            try:
                ...
                if (
                    row["properties"]["AYB"] is not None
                    and int(row["properties"]["AYB"]) > current_year
                ):
                    continue
                ...
                for zone in zone_types:
                    if zone == "anc":
                        zone_count = anc_count
                    elif zone == "census_tract":
                        zone_count = census_count
                    elif zone == "neighborhood_cluster":
                        zone_count = cluster_count
                    elif zone == "ward":
                        zone_count = ward_count
                    elif zone == "zip":
                        zone_count = zipcode_count
                    ...
                    if "Warning" not in mar_return.keys():
                        ...
                        for (
                            ...
                            zone_count
                        ):  # dictionary is {'zone_type': 'ANC', 'zone': 'ANC 8A', etc.}
                            if (
                                dictionary["zone"] == mar_return[zone]
                            ):  # mar_return[ANC] is 'ANC 8A'
                        ...
                        if not flag:
                            zone_count.append(
                                OrderedDict(
                                    [
                                        ("zone_type", zone),
                                        ("zone", mar_return[zone]),
                                        ("housing_unit_count", num_units),
                                        ("bedroom_unit_count", bedrm),
                                    ]
                                )
                            )
                ...
                print(
                    "Error! SSL: ", row["properties"]["SSL"], row["properties"]["AYB"]
                )
                continue

        return {
            "anc": anc_count,
            "census_tract": census_count,
            "neighborhood_cluster": cluster_count,
            "ward": ward_count,
            "zip": zipcode_count,
        }
    ...
    def get_csv(self):
        ...
        data_processed_zoneUnitCount = os.path.join(
            PYTHON_PATH, os.pardir, "data", "processed", "zoneUnitCount"
        )
        ...
        filename = os.path.join(
            data_processed_zoneUnitCount, "zoneUnitCount_" + date + ".csv"
        )
