...
from housinginsights.sources.models.DCHousing import FIELDS, DCHousingResult
from housinginsights.sources.models.DCHousing import (
    PROJECT_FIELDS_MAP,
    SUBSIDY_FIELDS_MAP,
...
class DCHousingApiConn(ProjectBaseApiConn):
    ...
    API_BASEURL = (
        "http://maps2.dcgis.dc.gov/dcgis/rest/services/DCGIS_DATA/"
    ...
    def __init__(self, baseurl=None, proxies=None, database_choice=None, debug=False):
        super().__init__(
            baseurl=DCHousingApiConn.BASEURL,
            proxies=proxies,
    ...
    def get_data(self, unique_data_ids=None, sample=False, output_type="csv", **kwargs):
        ...
        for u in unique_data_ids:
            if u not in self._available_unique_data_ids:
                ...
                logger.info(
                    "  The unique_data_id '%s' is not supported by the DCHousingApiConn",
                    u,
                )
                ...
                if output_type == "stdout":
                    ...
                    self.directly_to_file(data=content, filepath=self.output_paths[u])
                    self.create_project_subsidy_csv(
                        self._available_unique_data_ids[0],
                        PROJECT_FIELDS_MAP,
                        SUBSIDY_FIELDS_MAP,
                        PROJECT_ADDRE_FIELDS_MAP,
                        db,
