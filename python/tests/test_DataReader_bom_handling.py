...
meta_path = os.path.abspath(
    this_dir + "/test_data/meta_bom_handling.json"
)  # TODO Update when meta.json is updated.
manifest_path = os.path.abspath(
    this_dir + "/test_data/manifest_bom_handling.csv"
)  # TODO Consider updating manifest_sample
...
LOG_MSG_MATCH_DEFAULT_TO_LOCAL = (
    "Defaulting to local data file"  # DataReader:_download_data_file()
)
LOG_MSG_MATCH_ATTEMPT_TO_DOWNLOAD = (
    "Attempting to download file to disk"  # DataReader:_download_data_file()
)
LOG_MSG_MATCH_DOWNLOAD_COMPLETE = (
    "Download complete"  # DataReader:_download_data_file()
)
LOG_MSG_MATCH_REMOTE_FILE_ACCESS = (
    "File will be read from remote location"  # DataReader:_set_keys()
)
LOG_MSG_MATCH_LEADING_BOM_FOUND = (
    "Leading BOM (byte order mark) found in file"  # DataReader:_set_keys()
)
LOG_MSG_MATCH_HEADER_FIELDS_MATCH = (
    "meta.json and csv field lists match completely"  # DataReader:_do_fields_match()
)
            ...
            if manifest_row["include_flag"] == "use":
                ...
                if sample_data_folder_path[:3] == "../":
                    ...
                    sample_data_file_path = (
                        sample_data_folder_path + "/" + manifest_row["filepath"]
                    )
                else:
                    print(
                        "  Unexpected local sample data file relative path : '{}'".format(
                            sample_data_file_path
                        )
                    )
                ...
                if os.path.isfile(sample_data_file_path):
                    os.replace(
                        sample_data_file_path, sample_data_file_path + backup_suffix
                    )
        ...
        for manifest_row in manifest:
            if manifest_row["include_flag"] == "use":
                ...
                if os.path.isfile(backup_sample_data_file_path):
                    truncate_index = len(backup_sample_data_file_path) - len(
                        backup_suffix
                    )
                    orig_sample_data_file_path = backup_sample_data_file_path[
                        :truncate_index
                    ]
...
class TestBomHandling(BomHandlingCase):
            ...
            if manifest_row["include_flag"] == "use":
                ...
                if "_bom" in unique_data_id:
                    ...
                    with self.subTest(subtest="1a"):
                        subtest1a_success = self._test_bom_handling_case_1a(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest1a_success, "If failed, test case 1a failed.\n"
                        )
                    ...
                    with self.subTest(subtest="1b"):
                        subtest1b_success = self._test_bom_handling_case_1b(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest1b_success, "If failed, test case 1b failed.\n"
                        )

                    # Subtest Case 1c: File with bom in header row:
                    #                  load_from='s3': DataReader reads from remote file
                    with self.subTest(subtest="1c"):
                        subtest1c_success = self._test_bom_handling_case_1c(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest1c_success, "If failed, test case 1c failed.\n"
                        )
                ...
                # Test Case 2: Test sample data file without bom in header row
                    ...
                    with self.subTest(subtest="2a"):
                        subtest2a_success = self._test_bom_handling_case_2a(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest2a_success, "If failed, test case 2a failed.\n"
                        )
                    ...
                    with self.subTest(subtest="2b"):
                        subtest2b_success = self._test_bom_handling_case_2b(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest2b_success, "If failed, test case 2b failed.\n"
                        )

                    # Subtest Case 2c: File without bom in header row:
                    #                  load_from='s3': DataReader reads from remote file
                    with self.subTest(subtest="2c"):
                        subtest2c_success = self._test_bom_handling_case_2c(
                            manifest_row, local_sample_data_file
                        )
                        self.assertTrue(
                            subtest2c_success, "If failed, test case 2c failed.\n"
                        )
    ...
    def _test_bom_handling_case_1a(self, manifest_row, local_sample_data_file):
        ...
        print(
            "                     load_from='file': Sample data file does not already \n"
        ...
        case_name = (
            "DataReader instantiation for input '{}' with BOM header, "
            "triggered remote download case.".format(unique_data_id)
        )

        self.assertFalse(
            os.path.isfile(local_sample_data_file),
        ...
        with self.assertLogs(level="INFO") as logged:
            csv_reader = DataReader(
                meta=meta, manifest_row=manifest_row, load_from="file"
            )
        ...
        if len(logged.output) == 4:
            self.assertIn(
                LOG_MSG_MATCH_HTTP_HDR_CONTENT_TYPE,
                logged.output[0],
        ...
        with open(
            local_sample_data_file, mode="r", encoding=manifest_row["encoding"]
        ) as f:
    ...
    def _test_bom_handling_case_1c(self, manifest_row, local_sample_data_file):
        ...
        with self.assertLogs(level="INFO") as logged:
            csv_reader = DataReader(
                meta=meta, manifest_row=manifest_row, load_from="s3"
            )
        self.assertIsInstance(
            csv_reader,
            DataReader,
        ...
        s3_path = os.path.join(
            manifest_row["s3_folder"], manifest_row["filepath"].strip("\/")
        ).replace("\\", "/")
    ...
    def _test_bom_handling_case_2a(self, manifest_row, local_sample_data_file):
        ...
        case_name = (
            "DataReader instantiation for input '{}' without BOM header, "
            "triggered remote download case.".format(unique_data_id)
        )
