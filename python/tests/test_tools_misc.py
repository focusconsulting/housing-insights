...
class ToolsMiscTestCase(unittest.TestCase):
    ...
    def test_get_unique_addresses_from_str(self):
        ...
        address_str = (
            "1110 Aspen Street NW; 6650 Georgia Avenue NW; "
            "6656 Georgia Avenue NW; others"
        )
        expected_result = [
            "1110 Aspen Street NW",
            "6650 Georgia Avenue NW",
            "6656 Georgia Avenue NW",
            "others",
        ]
        ...
        self.assertEqual(
            len(expected_result),
            len(result),
            'Failed ";" - returned less/more unique ' "addresses {}".format(result),
        )
        for address in result:
            self.assertTrue(
                address in expected_result,
        ...
        self.assertEqual(
            len(expected_result),
            len(result),
            'Failed " and " - returned less/more unique ' "addresses {}".format(result),
        )
        for address in result:
            self.assertTrue(
                address in expected_result,
        ...
        expected_result = ["1309 E Street SE", "1311 E Street SE", "1313 E Street SE"]
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(result, 'Failed odd "-" - returned empty list')
        self.assertEqual(
            len(expected_result),
            len(result),
            'Failed odd "-" - returned less/more unique ' "addresses {}".format(result),
        )
        for address in result:
            self.assertTrue(
                address in expected_result,
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed even "-" delimiter case: {}'.format(address),
            )
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed ambiguous "-" delimiter case: {}'.format(address),
            )
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                "Failed '&' + 'and' delimiter " "composite: {}".format(address),
            )

        # case 6: both addresses delimiter and address number ranges
        address_str = (
            "1110 Aspen Street NW; 4000-4005 8th Street NE; "
        ...
        self.assertTrue(
            result,
            "Failed both number range and multiple "
        ...
        address_str = (
            "4232-4238 4th Street SE and 4281-4285 6th Street SE; "
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(
            result, "Failed composite of many delimiter - returned " "empty list"
        )
        self.assertEqual(
            len(expected_result),
            len(result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                "Failed composite of many delimiter " "cases: {}".format(address),
            )
        ...
        self.assertEqual(
            address_str,
            result[0],
            "Failed single address validation: {}".format(result[0]),
        )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(result, "Failed remove extra comma - returned " "empty list")
        self.assertEqual(
            len(expected_result),
            len(result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                "Failed remove extra comma case: {}".format(address),
            )
        ...
        expected_result = ["230 BATES ST NW", "24 BATES ST NW", "52 BATES ST NW"]
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(result, 'Failed "&" + comma composite - returned empty ' "list")
        self.assertEqual(
            len(expected_result),
            len(result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed "&" and comma composite: {}'.format(address),
            )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(result, 'Failed funky "&" clean up - returned empty ' "list")
        self.assertEqual(
            len(expected_result),
            len(result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed funky "&" clean up case: {}'.format(address),
            )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(
            result, 'Failed "-" + "&" + comma composite - returned ' "empty list"
        )
        self.assertEqual(
            len(result),
            len(expected_result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed "-" + "&" + comma composite ' "case: {}".format(address),
            )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(result, 'Failed handling invalid "&" - ' "returned empty list")
        self.assertEqual(
            len(result),
            len(expected_result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                'Failed handling invalid "&" case: {}'.format(address),
            )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(
            result, "Failed incomplete dash address number - " "returned empty list"
        )
        self.assertEqual(
            len(result),
            len(expected_result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                "Failed incomplete dash address number " "case: {}".format(address),
            )
        ...
        result = misc_tools.get_unique_addresses_from_str(address_str)
        self.assertTrue(
            result, "Failed incomplete comma address number - " "returned empty list"
        )
        self.assertEqual(
            len(result),
            len(expected_result),
        ...
        for address in result:
            self.assertTrue(
                address in expected_result,
                "Failed incomplete comma address number " "case: {}".format(address),
            )
        ...
        self.assertTrue(
            result,
            "Failed incomplete ampersand address number - " "returned empty list",
        )
        self.assertEqual(
            len(result),
            len(expected_result),
        ...
        self.assertTrue(
            result,
            "Failed multiple contiguous range delimiter - " "returned empty list",
        )
        self.assertEqual(
            len(result),
            len(expected_result),
