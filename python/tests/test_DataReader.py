...
meta_path = os.path.abspath(
    this_dir + "/test_data/meta_sample.json"
)  # TODO Update when meta.json is updated.
manifest_path = os.path.abspath(
    this_dir + "/test_data/manifest_sample.csv"
...
class TestVerifyRequiredMetadata(unittest.TestCase):
    def test_verify_required_metadata_columns(self):
        ...
        # Baseline assertions
        self.assertTrue(
            csv_reader._verify_required_metadata_columns_exist_in_actual(field_list)
        )
...
def prep_sql_manifest_row(manifest_row):
    temp_filepath = os.path.abspath(
        os.path.join(logging_path, "temp_{}.psv".format(manifest_row["unique_data_id"]))
    )
    sql_interface = HISql(
        meta=meta, manifest_row=manifest_row, engine=engine, filename=temp_filepath
    )
