...
def get_multiple_api_sources(a):
    ...
    for m in module_list:
        try:
            ...
            )  # Every class should have a get_data method!

            # Get the data
            api_method(
                unique_data_ids, sample, output_type, db=db
            )  # TODO refactor all the methods that need db to instead use self.engine() created in __init__(see base_project for example)
    ...
    # update the manifest
    manifest = Manifest(
        os.path.abspath(os.path.join(python_filepath, "scripts", "manifest.csv"))
    )
...
parser.add_argument(
    "database",
    help="""which database we should connect to 
...
parser.add_argument(
    "-s",
    "--sample",
    help="""Only download a sample of data. Used
...
parser.add_argument(
    ...
    choices=["opendata", "DCHousing", "dhcd", "census", "wmata_distcalc", "prescat"],
)
