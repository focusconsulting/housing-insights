...
def send_log_file_to_admin(debug=True):
    ...
    email.message = "See attached for the logs from the weekly_update(). Log counts by level are as follows: {}".format(
        level_counts
    )
...
if __name__ == "__main__":
    services_parser = argparse.ArgumentParser(
        "Services.py for running the weekly update job"
    )
    ...
    services_parser.add_argument(
        "--drop-all",
        help="drop all tables before starting the update",
        action="store_true",
    )
