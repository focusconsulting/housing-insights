...
def get_db_squares():
    ...
    query_result = database_connection.execute(
        "SELECT DISTINCT substring(ssl FROM 1 for 4) FROM dc_tax;"
    )
    ...
    for row in query_result:
        ...
        for value in row:
            urls.append(
                "https://maps2.dcgis.dc.gov/dcgis/rest/services/DCOZ/Zone_Mapservice/MapServer/25/query?f=json&where=Square ="
                + "'"
                + value
                + "'"
...
def fetch_zoning_data(urls):
    ...
    zoning_data = {"empty_zoning_data": [], "existing_zoning_data": []}
