...
def findRailStations(
    railStations, project, radiusinmeters, distCsvWriter, mapbox_api_key
):
    ...
    for station in railStations:
        walkDist = getWalkingDistance(
            lat, lon, str(station["Lat"]), str(station["Lon"]), mapbox_api_key
        )
        if walkDist <= radiusinmeters:
            distCsvWriter.writerow(
                (
                    project["Nlihc_id"],
                    "rail",
                    station["Code"],
                    "{0:.2f}".format(getMiles(walkDist)),
                )
            )


def findBusStations(
    project, radiusinmeters, distCsvWriter, wmata_api_key, mapbox_api_key
):
    ...
    for stop in data["Stops"]:
        walkDist = getWalkingDistance(
            lat, lon, str(stop["Lat"]), str(stop["Lon"]), mapbox_api_key
        )
...
def writeBusInfo(infoCsvWriter, wmata_api_key):
    ...
    for stop in data["Stops"]:
        ...
        infoCsvWriter.writerow(
            (stop["StopID"], "bus", stop["Name"], stop["Lat"], stop["Lon"], lines)
        )
...
def main(secretsFileName, csvInputFileName, distOutputFileName, infoOutputFileName):
    ...
    distCsvWriter.writerow(
        ("Nlihc_id", "type", "stop_id_or_station_code", "dist_in_miles")
    )
