...
def construct_project_view_blueprint(name, engine):
    ...
    def nearby_building_permits(dist):
        ...
        latitude_tolerance, longitude_tolerance = bounding_box(
        ...
        good_results = [
            dict(r)
            for r in results
            if haversine(latitude, longitude, float(r.latitude), float(r.longitude))
            <= dist
        ]

        tot_permits = len(good_results)

        output = {"objects": good_results, "tot_permits": tot_permits, "distance": dist}
    ...
    def nearby_projects(dist):
        ...
        unit_counts = [
            int(u) for u in unit_counts
        ]  # temporarily needed b/c accidentally stored as text
