...
## 1) Confirm what datasets are ready, which need work, and mark them accordingly to help the data team

At this time, we have identified a series of datasets, the priority of Housing Insights shareholders for respective datasets, and how far along contributors are in preparing one.

Save yourself duplicate effort and maximize your effectiveness by reviewing the [Requsted Data Status spreadsheet on Google Docs](https://docs.google.com/spreadsheets/d/1UnuTvDRrCya3gWaNTfDQfTQPRwuwAqB6p9zzlCLDh_g/edit#gid=264737146).

Familiarizing yourself now will help, because you will need to update this spreadsheet later.

## 2) Download the raw data file for your new data source
...
## 3) Add a new table to `table_info.json` (renaming in progress - previously `meta.json`)
...
## 4) Add the file to `manifest.csv`
...
## 8) Update Status of Your Dataset

To help us better understand what is needed and to coordinate with the different functional teams, update [the Requested Data Status spreadsheet](https://docs.google.com/spreadsheets/d/1UnuTvDRrCya3gWaNTfDQfTQPRwuwAqB6p9zzlCLDh_g/edit#gid=264737146).

- On the first sheet, Requested Data Status, locate the dataset. At a minimum, please fill in the following columns:
  - `topic_to_pg_table`: the table in the database where the data will reside
  - `pg_table_to_raw_dataset`: The path to the directory in `raw/` and file which will be saved in the `manifest` table for in application tracking
  - `status`: Change this from `Checked` to `Loaded` once research and evaluation is complete and finished the ingestion scripts

- Update the `raw_dataset_checklist` sheet to indicate the necessary requirements for your ingestion scripts are met. Mark `Y` if defined and `N` for not.
  - `raw_dataset`: the path to the directory and file used for ingestion (see `pg_table_to_raw_dataset` above)
  - `nhilc_normalized`: the National Low Income Housing Coalition ID, normalized to properly join across tables (if you do not know, ask)
  - `ward_normalized`: the DC Ward normalized to the proper numeric format to join across tables
  - `tract_normalized`: the DC tract ID normalized for the proper format to join across tables

- Update the `filters` sheet to indicate if your dataset and your ingestion scripts handle the appropriate filters

## 9) Upload the data to S3
...
## 10) Commit your changes in Git and push them to Github.
