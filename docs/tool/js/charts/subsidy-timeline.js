...
var SubsidyTimelineChart = function (chartOptions) {
  ...
  this.extendPrototype(
    SubsidyTimelineChart.prototype,
    subsidyTimelineExtension
  );
...
var subsidyTimelineExtension = {
  // Final step of inheriting from Chart, defines the object with which to extend the prototype
  ...
  initialize: function (chartOptions) {
    ...
    chart.svg = d3
      .select(chartOptions.container) // select elem (div#chart-0)
    ...
    function doStuffWithData(returnData) {
      ...
      function nullJudgment() {
        ...
        const noSubsidyHasDate = data.every(
          (subsidy) => subsidy['poa_start'] === null
        );
        if (noSubsidyHasDate) {
          this.message =
            'Chart cannot be displayed; subsidy data does not contain start and end dates.';
          this.status = true;
        }
      }
      ...
      if (isNull.status === true) {
        ...
        chart.setupWithData(
          chart,
          data.filter((subsidy) => subsidy['poa_start'] !== null)
        );
      }
    }
  }, // end setup
  ...
  setupWithData: function (chart, data) {
    ...
    chart.xScale = d3
      .scaleTime()
      .domain([new Date(chart.minTime), new Date(chart.maxTime)])
      .range([0, 0.9 * chart.width]);
    ...
    groups = chart.svg.selectAll('g').data(data).enter().append('g');
  ...
  timeParser: {
    // This is very data format dependent, and will need to be replaced when we have
    // a final data format
